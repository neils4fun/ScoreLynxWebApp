          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {leaders.map((team, index) => (
              <tr key={team.teamID} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                <td className="px-2 py-2 text-sm text-gray-900">{team.teamName}</td>
                ...
                <td className="px-2 py-2 text-sm text-center">{team.absoluteScore}</td>
                <td className="px-2 py-2 text-sm text-center">{team.holesPlayed}</td>
              </tr>
            ))}
          </tbody>
        </table>
